...
<head>
    ...
    <title>Edit Siswa - SiTelat K4</title>
...
<body class="bg-slate-50 text-slate-800 antialiased min-h-screen flex flex-col">
    ...
    <header class="bg-white border-b border-slate-200 sticky top-0 z-30 px-6 py-4 flex justify-between items-center shadow-sm">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-lg font-bold tracking-tight">Edit Siswa</h1>
                <p class="text-[11px] text-slate-400">Modifikasi biodata siswa terdaftar</p>
            </div>
        </div>
        <div>
        ...
        <main class="flex-grow p-8">
            <div class="max-w-2xl mx-auto">
                <div class="bg-white border border-slate-200 rounded-3xl p-8 shadow-xl shadow-slate-100/50">
                    ...
                    <div class="mb-6">
                        <h2 class="text-xl font-bold text-slate-800">Ubah Biodata Siswa</h2>
                        <p class="text-xs text-slate-400 mt-1">Harap sesuaikan biodata siswa dengan data yang valid.</p>
                    </div>
                    ...
                    <form action="{{ route('superadmin.siswa.update', $siswa->id) }}" method="POST" class="space-y-5">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <input type="text" id="nisn" name="nisn" required value="{{ old('nisn', $siswa->nisn) }}" placeholder="Contoh: 0098231221"
                        ...
                        <div>
                            ...
                            <input type="text" id="nama" name="nama" required value="{{ old('nama', $siswa->nama) }}" placeholder="Contoh: Ahmad Fauzi"
                                class="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-3 text-sm text-slate-800 focus:bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 focus:outline-none transition-all duration-200">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" id="kelas" name="kelas" required value="{{ old('kelas', $siswa->kelas) }}" placeholder="Contoh: XI RPL 1"
                                class="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-3 text-sm text-slate-800 focus:bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 focus:outline-none transition-all duration-200">
                        </div>
                        ...
                        <div>
                            ...
                            <select id="jurusan" name="jurusan" required
                                ...
                                <option value="PPLG" {{ old('jurusan', $siswa->jurusan) === 'PPLG' ? 'selected' : '' }}>PPLG (RPL)</option>
                                ...
                                <option value="TPFL" {{ old('jurusan', $siswa->jurusan) === 'TPFL' ? 'selected' : '' }}>TPFL</option>
                            </select>
                        </div>
