...
<head>
    ...
    <title>Edit Akun - SiTelat K4</title>
...
<body class="bg-slate-50 text-slate-800 antialiased min-h-screen flex flex-col">
    ...
    <header class="bg-white border-b border-slate-200 sticky top-0 z-30 px-6 py-4 flex justify-between items-center shadow-sm">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-lg font-bold tracking-tight">Edit Akun</h1>
                <p class="text-[11px] text-slate-400">Modifikasi data & akses akun</p>
            </div>
        </div>
        <div>
        ...
        <main class="flex-grow p-8">
            <div class="max-w-2xl mx-auto">
                <div class="bg-white border border-slate-200 rounded-3xl p-8 shadow-xl shadow-slate-100/50">
                    ...
                    <div class="mb-6">
                        <h2 class="text-xl font-bold text-slate-800">Ubah Data Akun</h2>
                        <p class="text-xs text-slate-400 mt-1">Kosongkan kolom kata sandi jika Anda tidak ingin mengubahnya.</p>
                    </div>
                    ...
                    <form action="{{ route('superadmin.users.update', $user->id) }}" method="POST" class="space-y-5">
                        ...
                        <div>
                            ...
                            <input type="text" id="name" name="name" required value="{{ old('name', $user->name) }}" placeholder="Contoh: Budi Santoso"
                        ...
                        <div>
                            ...
                            <input type="email" id="email" name="email" required value="{{ old('email', $user->email) }}" placeholder="Contoh: budi@sitelat.com"
                                class="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-3 text-sm text-slate-800 focus:bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 focus:outline-none transition-all duration-200">
                        </div>
                        ...
                        <div>
                            ...
                            <select id="role" name="role" required onchange="toggleKelasField()"
                                class="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-3 text-sm text-slate-800 focus:bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 focus:outline-none transition-all duration-200">
                                <option value="admin" {{ old('role', $user->role) === 'admin' ? 'selected' : '' }}>OSIS (Admin)</option>
                                <option value="user" {{ old('role', $user->role) === 'user' ? 'selected' : '' }}>Wali Kelas (User)</option>
                                <option value="super_admin" {{ old('role', $user->role) === 'super_admin' ? 'selected' : '' }}>Super Admin</option>
                            </select>
                        </div>

                        <!-- Kelas (Khusus Walas) -->
                        <div id="kelas_container" class="{{ old('role', $user->role) === 'user' ? '' : 'hidden' }}">
                        ...
                        <!-- Password -->
                        <div>
                            <label for="password" class="block text-sm font-semibold text-slate-600 mb-1.5">Kata Sandi Baru</label>
