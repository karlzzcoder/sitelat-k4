...
<body class="bg-slate-50 text-slate-800 antialiased min-h-screen flex flex-col">
    ...
    <header class="bg-white border-b border-slate-200 sticky top-0 z-30 px-6 py-4 flex justify-between items-center shadow-sm">
        <div class="flex items-center gap-3">
            <a href="{{ route('superadmin.keterlambatan.index') }}" class="w-8 h-8 rounded-lg border border-slate-200 flex items-center justify-center text-slate-400 hover:text-slate-600 transition-colors">
                <i class="fa-solid fa-arrow-left"></i>
            </a>
            <div>
                <h1 class="text-lg font-bold tracking-tight">Ubah Catatan Keterlambatan</h1>
                <p class="text-[11px] text-slate-400">Modifikasi data keterlambatan master</p>
            </div>
        </div>
        <div>
    ...
    <div class="flex-grow flex">
        ...
        <aside class="w-64 bg-white border-r border-slate-200 hidden md:block p-6 space-y-6">
            <div class="space-y-1">
                ...
                <a href="{{ route('superadmin.keterlambatan.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl bg-indigo-50 text-indigo-600 font-bold text-sm transition-all">
        ...
        <main class="flex-grow p-8">
            <div class="max-w-2xl mx-auto">
                <div class="bg-white border border-slate-200 rounded-3xl p-8 shadow-xl shadow-slate-100/50">
                    ...
                    <div class="mb-6">
                        <h2 class="text-xl font-bold text-slate-800">Form Koreksi Catatan</h2>
                        <p class="text-xs text-slate-400 mt-1">Sesuaikan tanggal, alasan, atau keterangan penanganan.</p>
                    </div>
                    ...
                    <form action="{{ route('superadmin.keterlambatan.update', $riwayat->id) }}" method="POST" class="space-y-5">
                        @csrf
                        @method('PUT')
                        ...
                        <!-- Alasan -->
                        <div>
                            <label for="alasan" class="block text-sm font-semibold text-slate-600 mb-1.5">Alasan Keterlambatan <span class="text-rose-500">*</span></label>
                        ...
                        <!-- Penanganan -->
                        <div>
                            <label for="penanganan" class="block text-sm font-semibold text-slate-600 mb-1.5">Tindakan Penanganan Wali Kelas</label>
                            <textarea id="penanganan" name="penanganan" rows="3" placeholder="Tulis penanganan dari Walas..."
                                class="w-full bg-slate-50 border border-slate-300 rounded-xl px-4 py-3 text-sm text-slate-800 focus:bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 focus:outline-none transition-all duration-200 resize-none">{{ old('penanganan', $riwayat->penanganan) }}</textarea>
                        </div>

                        <!-- Submit Buttons -->
                        <div class="flex gap-4 pt-2">
                            <a href="{{ route('superadmin.keterlambatan.index') }}" 
